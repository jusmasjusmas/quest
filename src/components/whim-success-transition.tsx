...
export function WhimSuccessTransition({
  ...
  useLayoutEffect(() => {
    ...
    // Wait two frames so width and text metrics match the visible layout viewport.
  ...
  return (
    ...
    >
      <div className="relative flex h-full min-h-0 w-full min-w-0 flex-col overflow-hidden">
        <div className="pointer-events-none absolute inset-x-0 top-1/2 w-full min-w-0 -translate-y-1/2">
          <motion.span
            ref={textRef}
            animate={xControl}
            style={{ opacity: phase2 ? 0 : 1 }}
            className="inline-block whitespace-nowrap font-serif text-[min(22vw,6.75rem)] italic leading-none text-white transition-opacity duration-500 ease-in-out will-change-transform sm:text-[7.5rem]"
          >
            Sweeeeeeeeet
          </motion.span>
        </div>
        ...
        {phase2 ? (
          <motion.div
            className="absolute inset-0 z-[1] flex flex-col items-start justify-center px-7 sm:px-8"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
